...
frameworks in which multi-platform applications could be executed. An example of this is the Flash Player experience, since its usage were drastically reduced by HTML when de browsers were able to do the work \cite{Winokur11}. 
But, Nowadays, most programs executed by browsers are written in a language
...
% Add to bibliography if needed - JJ- añadida, Maribel, y he añadido una frase en este primer párrafo haciendo referencia a Flash
...
\section{The {\em jsEvRBF} implementation} %Is it an algorithm or an
                                %implementation? - JJ, Yo creo que es una implementación, el algoritmo no es nuevo.
\label{sec:algorithm}
% The {\em EvRBF} general skeleton
{\em jsEvRBF} is an evolutionary algorithm implemented in JavaScript, so
that it can be executed in web browsers.
%Once again, you are mixing implementation and algorithm. Is it an
%algorithm or an implementation? ~ JJ, hecho, Maribel
As in {\em EvRBF} \cite{rivas03:EvRBF}, its predecessor, individuals are complete RBFNN, and
...
{\em jsEvRBF} is a generational algorithm, with a fixed number of
...
The data-set is composed of 208
