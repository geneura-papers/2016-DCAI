...
%
\title{TITULO VIEJO: Web browser-based forecasting of economic time-series ######BUSCAR NUEVO TITULO####}
%
\titlerunning{TITULO VIEJO: Web browser-based forecasting ######BUSCAR NUEVO TITULO####}  % abbreviated title (for running head)
...
\begin{abstract}
%##### REESCRIBIR ABSTRACT CUANDO HAYAMOS HECHO TODO LO DEMAS
...
exchange rates between two currencies, for a four years period (data
is weekly averaged). On the other hand, the algorithm (described in
section \ref{sec:algorithm}) is a reduced version of {\em EvRBF}
\cite{Rivas04}, an evolutionary algorithm that makes Radial Basis
...
that it can be executed in web browsers.  As in {\em EvRBF}
\cite{Rivas04}, its predecessor, individuals are complete RBFNN, and
special operators have been created to cross and mutate them. {\em
...
and compares with results from previous papers by Rivas \cite{Rivas04}
...
\caption{First experiment: comparison of MSE yielded by the {\em
    jsEvRBF} algorithm and the ones cited in \cite{Rivas04}. Errors
