...
In order to prove the performance in a volunteer computing set up, we
...
participated in it. Besides, and in order to have an indea of the
baseline performance of every browser, we have
...
Our intention with this experiment was to measure the performance of
single browsers, which arises from the implementation of the
JavaScript virtual machine they use. It will give us a base of
comparison with other languages or frameworks, and can be used also to
decide on a particular browser in case that aspect of the experiment
can be controlled. The configuration, similarly to that used in
\cite{rivas03:EvRBF}, was % This has to be explicited, a citation is
                          % not enough. 
. These experiments used the algorithm configuration shown in Table
\ref{tab:parameters-second-experiment}. % Say something about this
                                % configuration. Is it average? Good?
                                % Bad? - JJ
